...
- 第六步，运行 `scripts/audit_readme.py <仓库路径> --strict-warnings`，检查双语结构、一级大标题居中、章节编号、题注位置、列表缩进、流程图、术语、中文标点、链接、秘密、用户目录路径、SVG 活动内容、外部引用和图片元数据，修复全部硬错误并人工复核提醒
...
- 三个以上流程节点、条件分支、失败重试、多个参与方或返回前一步时提供竖向 Mermaid 图，一至两个简单动作继续使用文字
- 影响读者操作的技术词首次出现时说明定义、用途、可观察表现和结果；官方拼写保持不变，npm 使用小写，WebP 不编造全称
- 中文说明按照现象或事件、直接原因、结果或影响、补充解释或下一步展开
- 普通中文正文禁止中文句号，句内使用中文分号，段落末尾不保留中文句号或中文分号；引文、日志和代码保持原样
